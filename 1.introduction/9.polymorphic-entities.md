...
See also:
  - [Basic entities](./8.entities.md)
  - [Root aggregates](./10.root-aggregates.md)

It is possible to make domain objects with inheritance. Polymorphic relations with interfaces are not supported. To create a polymorphic relation we need to create a base class that implements `PolymorphicEntityInterface`.
...
Technically it would work, but full support is not guaranteed.
```php
use Apie\Core\Entities\PolymorphicEntityInterface;
use Apie\Core\Other\DiscriminatorConfig;
use Apie\Core\Other\DiscriminatorMapping;
